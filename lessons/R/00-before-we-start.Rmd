...
* Once the url is copy/pasted, the `Project directory name` will automatically populate with the name of the repo `2015-03-23-hbc-swc`. You will need to specify where you would like to place thsi directory. This will become your **working directory** for the rest of class.
...
instance `read.csv(file="put_some_data_here")`.
...
Let's look at a simple function call below `read.csv`. This function is used to read in data from a csv file.

```{r, eval=FALSE}
metadata <- read.csv(file="meta/mouse_exp_design.csv")
```
_Autocomplete: When typing out the function call try pressing the `Tab` key after typing only `read`. You will see a drop-down menu appear offering you a list of read functions that are available for use. The window to the right gives you more information on the function and its arguments as you scroll down and highlight each individually._ 
...
Once you feel you have mastered some of the basics, you may want to check out Hadley Wickham's [online book](http://adv-r.had.co.nz/) to improve programming skills and better undertand the R language.
