...
However, to do useful and interesting things, we need to **assign values to objects**. To create objects, we need to give it a name followed by the
...
_Note: When typing out a variable that is stored try pressing the `Tab` key after typing only `weight`. You will find that RStudio will auto-complete the variable name or a drop-down menu will appear. For function calls this will also work. The window to the right gives you more information on the function and its arguments as you scroll down and highlight each individually._ 
...
Let's look at a more advanced function call below `read.csv`. This function is used to read in data from a csv file.
...
There are a set of standard (or base) packages which are considered part of the R source code. They contain the basic functions that allow R to work, and enable standard statistical and graphical functions on datasets. They should be automatically available in any R installation. The functions that we have been using so far in our examples are from the base packages.

You can check this by typing into the console and listing your attached base packages:

```{r, eval=FALSE}
sessionInfo()
```

For the most part we will be using functions from the standard base packages. However, the more you work with R you will come to realize that going beyond the standard there is a world of added functionality. To use additional packages you will first need to install it, and then load the library for use in your R session. 
Packages for R can be installed from the [CRAN](http://cran.r-project.org/) package repository:

```{r, eval=FALSE}
install.packages('package_name_here')
```

Alternatively, packages can also be installed from [Bioconductor]() by using the `biocLite.R` script 

```{r, eval=FALSE}
source("http://bioconductor.org/biocLite.R")
biocLite('package_name_here')
```
